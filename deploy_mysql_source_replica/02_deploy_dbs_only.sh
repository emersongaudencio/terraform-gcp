...
ansible -i ${OUTPUT_DIR}/db_hosts.txt -m shell -a "curl -sS https://raw.githubusercontent.com/emersongaudencio/general-deployment-scripts/master/automation/install_ansible_mysql_80.sh | sudo bash" dbservers -u $ansible_user --private-key=$priv_key --become -o > ${OUTPUT_DIR}/install_mariadb_dbservers.txt
...
ansible -i ${OUTPUT_DIR}/db_hosts.txt -m shell -a 'mysql -N -e "CHANGE MASTER TO master_host=\"{{ master_host }}\", master_port=3306, master_user=\"{{ master_user }}\", master_password = \"{{ master_password }}\", MASTER_AUTO_POSITION=0; CHANGE MASTER TO MASTER_LOG_FILE=\"{{ master_log_file }}\", MASTER_LOG_POS={{ master_log_pos }}; START SLAVE; SHOW SLAVE STATUS\G"' dbreplica01 -u $ansible_user --private-key=/root/repos/ansible_keys/ansible --become -e "{master_host: '$master_host', master_user: '$rep_user' , master_password: '$rep_pwd', master_log_file: '$log_file' , master_log_pos: '$log_position' }" -o > ${OUTPUT_DIR}/setup_replication_dbreplica01_activation.txt
ansible -i ${OUTPUT_DIR}/db_hosts.txt -m shell -a 'mysql -N -e "CHANGE MASTER TO master_host=\"{{ master_host }}\", master_port=3306, master_user=\"{{ master_user }}\", master_password = \"{{ master_password }}\", MASTER_AUTO_POSITION=0; CHANGE MASTER TO MASTER_LOG_FILE=\"{{ master_log_file }}\", MASTER_LOG_POS={{ master_log_pos }}; START SLAVE; SHOW SLAVE STATUS\G"' dbreplica02 -u $ansible_user --private-key=/root/repos/ansible_keys/ansible --become -e "{master_host: '$master_host', master_user: '$rep_user' , master_password: '$rep_pwd', master_log_file: '$log_file' , master_log_pos: '$log_position' }" -o > ${OUTPUT_DIR}/setup_replication_dbreplica02_activation.txt

ansible -i ${OUTPUT_DIR}/db_hosts.txt -m shell -a 'mysql -N -e "STOP SLAVE; SET GLOBAL gtid_purged=\"{{ gtid }}\"; CHANGE MASTER TO MASTER_AUTO_POSITION=1; START SLAVE; SHOW SLAVE STATUS;"' dbreplica01 -u $ansible_user --private-key=/root/repos/ansible_keys/ansible --become -e "{ gtid: '$gtid_position'}" -o > ${OUTPUT_DIR}/setup_replication_dbreplica01_activation_phase2.txt
ansible -i ${OUTPUT_DIR}/db_hosts.txt -m shell -a 'mysql -N -e "STOP SLAVE; SET GLOBAL gtid_purged=\"{{ gtid }}\"; CHANGE MASTER TO MASTER_AUTO_POSITION=1; START SLAVE; SHOW SLAVE STATUS;"' dbreplica02 -u $ansible_user --private-key=/root/repos/ansible_keys/ansible --become -e "{ gtid: '$gtid_position'}" -o > ${OUTPUT_DIR}/setup_replication_dbreplica02_activation_phase2.txt
